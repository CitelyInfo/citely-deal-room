...
const loginCss = `:root{font-family:Inter,-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;color:#1a1714;background:#f5f5f5}*{box-sizing:border-box}body{margin:0;min-height:100svh;display:grid;place-items:center;padding:28px}main{width:min(100%,440px);padding:40px;background:#fff;border:1px solid #dbdbdb;border-radius:16px;box-shadow:0 20px 60px #00000008}.brand{display:flex;align-items:center;gap:10px;font-weight:700;font-size:16px}.brand img{object-fit:contain}.eyebrow{font-size:12px;letter-spacing:2px;color:#8b1f30;margin:42px 0 14px}h1{font-size:29px;letter-spacing:-1px;margin:0 0 14px}p{color:#6f685c;font-size:15px;line-height:1.8}label{display:block;font-size:14px;margin:28px 0 10px}input,button{font:inherit;width:100%;min-height:48px;border-radius:7px}input{border:1px solid #b8b3ac;padding:12px;background:#fff;color:#1a1714}input:focus-visible,button:focus-visible{outline:3px solid #d29ba2;outline-offset:3px}button{margin-top:16px;border:0;background:#1a1714;color:#fff;cursor:pointer}button:hover{background:#8b1f30}.error{color:#8b1f30;background:#f8f1f2;border-radius:6px;padding:10px 12px;font-size:14px}.foot{font-size:12px;margin:22px 0 0;color:#6f685c}@media(max-width:480px){body{padding:18px}main{padding:28px}}`;

function loginPage(error = false) {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>受邀访问 · Citely</title><link rel="icon" href="/brand/citely-mark.png"><link rel="stylesheet" href="/invite.css"></head><body><main><div class="brand"><img src="/brand/citely-mark.png" width="24" height="24" alt="">Citely</div><p class="eyebrow">PRIVATE CASE STUDY</p><h1>欢迎来到 Citely 案例室</h1><p>输入你收到的邀请码，查看 WebMCP 交互案例。</p>${error ? '<p class="error" role="alert">邀请码无效，请检查后重试，或向邀请人获取最新邀请码。</p>' : ''}<form action="/invite" method="post"><label for="code">邀请码</label><input id="code" name="code" type="password" required maxlength="128" autocomplete="current-password" autocapitalize="none" spellcheck="false" placeholder="请输入邀请码"${error ? ' aria-invalid="true"' : ''}><button type="submit">进入案例室 →</button></form><p class="foot">没有邀请码？请联系向你分享此页面的人。</p></main></body></html>`;
}
...
export async function handleRequest(request, env, assets) {
  ...
  if (url.pathname === '/invite/logout' && request.method === 'POST') return redirect('/invite', clearCookie);
  if (url.pathname === '/invite' && request.method === 'POST') {
    ...
    if (!(await crypto.subtle.verify('HMAC', key, expected, encoder.encode(`invite:${code}`)))) {
      return respond(loginPage(true), 401, { 'Content-Type': 'text/html; charset=utf-8', 'Set-Cookie': clearCookie });
    }
    const expires = Math.floor(Date.now() / 1000) + SESSION_SECONDS;
    const signature = hex(await crypto.subtle.sign('HMAC', key, encoder.encode(`session:v1:${expires}:${env.INVITE_CODE}`)));
    return redirect('/case-study/', `${COOKIE}=${expires}.${signature}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${SESSION_SECONDS}`);
  }
  if (!readOnly) return respond('Method not allowed', 405, { Allow: 'GET, HEAD' });
  const authenticated = await validSession(request, env);
  if (url.pathname === '/invite') {
    return authenticated ? redirect('/case-study/') : respond(request.method === 'HEAD' ? null : loginPage(), 200, { 'Content-Type': 'text/html; charset=utf-8' });
  }
  if (!authenticated) return redirect('/invite');
