...
export type ErrorCode = 'HARD_STOP' | 'INVALID_INPUT' | 'HUMAN_CONFIRMED' | 'UNKNOWN_ID';
...
export function createTools(store: Store, now: () => string = () => new Date().toISOString()): ToolDef[] {
  ...
  const okRes = (extra: Record<string, unknown>): ToolResponse => ({ ok: true, ...base(), ...extra });
  const errRes = (code: ErrorCode, message: string): ToolResponse => ({ ok: false, ...base(), error: { code, message } });
  ...
  const getBlockers: ToolDef = {
    ...
    inputSchema: { type: 'object', additionalProperties: false, properties: {} },
    execute: async () => okRes({ doors: store.schema.doors, blockers: store.blockers() }),
  };
